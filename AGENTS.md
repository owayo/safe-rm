...
8. **シンボリックリンク経由の `..` 脱出防止**: `..` が既存 symlink 成分を消すパス（例: `link/../victim`）は削除前に拒否し、OS の path resolution と字句正規化の差でプロジェクト境界を脱出したり、正規化後の別ファイルを削除したりする挙動をブロック。メタデータ取得・削除・許可判定は引き続き `path_clean` で字句的に `..` を解決した正規化パス（`normalized_path`）で行う
