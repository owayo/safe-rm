...
6. **エイリアスパス耐性**: Gitチェックでは非symlinkパスを canonicalize して比較し、symlink パスは「親ディレクトリのみ canonicalize + リンク名維持」で照合することで、repo symlink 別名や `/var` と `/private/var` の差異によるバイパスを防止
