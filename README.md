...
- **Git Metadata Protection**: Always block `.git`, gitdir indirection files, bare-repository administrative paths, and recursive deletes that would include current-repository Git metadata
...
1. **Git Metadata Protection**: Always blocks `.git`, gitdir indirection files, bare-repository administrative paths, and recursive deletion of paths that contain current-repository Git metadata, even if config would otherwise allow deletion.
