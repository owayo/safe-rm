...
5. **Fail-Closed**: Any directory read failure (including entry iteration errors), Git API error from `Repository::discover()` on non-`allowed_paths` deletion paths (e.g. corrupted `.git`, permission errors, I/O failures), unreadable metadata probe while classifying a `NotFound`, an unresolvable intermediate symlink, or config file read/parse error blocks deletion or falls back to strict mode. `NotFound` is treated as non-Git only when no `.git`/bare-repository ancestor is found and the ancestor probe itself succeeds. `allowed_paths` still enforce Git metadata protection, but they do not require current-directory Git discovery to succeed
